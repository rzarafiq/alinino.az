...
//   const isAnyModalOpen = showForm || showWhatsapp || showFacebook || showInstagram || showYoutube || showX || showTelegram || showPinterest;
...
//       // Filter by title, author (characteristic), or ISBN (characteristic)
//       const results = allProductsData.filter(product =>
//         product.title?.toLowerCase().includes(term) ||
//         product.characteristics?.some(char =>
//           (char.property_id === 12057361 && char.title?.toLowerCase().includes(term)) || // Author ID
//           (char.property_id === 12019264 && char.title?.toLowerCase().includes(term))    // ISBN ID
//         )
//       );
...
//         // Optionally setIsActive(false) if you want to close overlay too
//         // But usually, we keep the input active if it has focus or value
//       }
//     };
...
//     (acc, product) => acc + (product.quantity || 1) * (product.variants?.[0]?.price || 0),
//     0
//   );
...
//               <ul ref={scrollRef} className="py-2 px-4 space-y-1 overflow-y-auto custom-scrollbar">
...
//           {/* <div className="relative w-full h-auto z-50" style={{ width: "clamp(430px, calc(430px + (580 * ((88vw - 1024px) / 270))), 784px)" }} >
//             {isActive && (
//               <div className="fixed inset-0 bg-[#0a0a0a98] z-60 transition-opacity duration-300"></div>
//             )}
...
//             {searchValue && (
//               <div className="mt-[136px] h-[500px] bg-white border border-gray-300 rounded shadow-lg overflow-y-auto z-50">
                
//                 <div className="p-4">Nəticələr: "{searchValue}"</div>
//               </div>
...
//                     <Link to={`/products/${product.permalink}`} className="text-[12px] text-[#000000] hover:text-[#f50809] transition-colors duration-300 font-normal">
//                       {product.title}
//                     </Link>
...
//       style={{
//         width: "clamp(430px, calc(430px + (580 * ((88vw - 1024px) / 270))), 784px)",
//       }}
//     >
//       {isActive && (
//         <div
//           className="fixed inset-0 bg-[#0a0a0a98] z-40 transition-opacity duration-300"
//           onClick={() => setIsActive(false)} // Optional: Clicking backdrop closes it
//         ></div>
//       )}

//       <div
//         ref={inputWrapperRef}
//         className={`relative h-[45px] rounded ${isActive ? "z-60 outline-4 outline outline-white" : ""}`}
//       >
//         <input
//           type="search"
//           placeholder="Kataloq üzrə axtarış"
...
//           onChange={(e) => setSearchValue(e.target.value)}
//           onFocus={() => setIsActive(true)}
//           // Optional: onBlur handling can be tricky with dropdowns, so often omitted or handled via click outside
//           className="w-full h-full px-4 pr-12 text-[14px] border-2 border-[#f50809] rounded outline-none bg-white relative z-10"
//         />
//         <div className="absolute top-0 right-0 h-full w-[50px] flex items-center justify-center bg-[#dc0708] rounded-r-md z-20">
//           <CgSearch className="text-[25px] text-white" />
...
//       {/* Axtarış nəticələri paneli */}
//       {/* Show results only if there's a search term and results exist, or if results exist (handles edge cases) */}
//       {(searchValue || filteredProducts.length > 0) && (
//         <div className="absolute left-0 right-0 mt-1 h-auto max-h-[500px] bg-white border border-gray-300 rounded shadow-lg overflow-y-auto z-50">
//           {filteredProducts.length > 0 ? (
//             filteredProducts.map((product) => (
//               <div key={product.id} className="flex items-center gap-3 w-full p-2 hover:bg-gray-100 cursor-pointer">
//                 <Link
//                   to={`/product/${product.permalink}`} // Adjust path as needed
//                   onClick={() => { setSearchValue(""); setIsActive(false); }} // Clear search and close on click
//                   className="flex items-center gap-3 w-full"
//                 >
//                   {/* Use the first available image or a placeholder */}
//                   <img
//                     src={product.images?.[0]?.compact_url || product.images?.[0]?.thumb_url || '/path/to/placeholder.jpg'}
//                     alt={product.title}
//                     className="w-10 h-16 object-cover flex-shrink-0"
//                     onError={(e) => { e.target.src = '/path/to/placeholder.jpg'; }} // Handle broken images
//                   />
//                   <div className="flex flex-col flex-1 min-w-0"> {/* min-w-0 prevents flex issues */}
//                     <span className="text-[12px] text-[#000000] hover:text-[#f50809] transition-colors duration-300 font-normal truncate">
//                       {product.title}
//                     </span>
//                     {/* Example: Display Author if available */}
//                     {/* Find author characteristic */}
//                     {(() => {
//                       const authorChar = product.characteristics?.find(char => char.property_id === 12057361); // Author ID
//                       return authorChar ? <span className="text-[10px] text-gray-600 truncate"> {authorChar.title}</span> : null;
//                     })()}
//                     <span className="text-[14px] text-[#000000] font-semibold">
//                       {product.variants?.[0]?.price} AZN
//                       {/* Optional: Show old price if available */}
//                       {product.variants?.[0]?.old_price && product.variants?.[0]?.old_price !== product.variants?.[0]?.price && (
//                         <span className="line-through text-[10px] text-gray-500 ml-1">
//                           {product.variants?.[0]?.old_price} AZN
//                         </span>
//                       )}
//                     </span>
//                   </div>
...
//               <div className="absolute -top-1.5 right-2 flex items-center justify-center text-[12px] text-[#ffffff] text-center bg-[#f50809] rounded-full min-w-[16px] h-4 px-1">
...
//                 {cartProducts.length === 0 ? "Səbət" : `${totalPrice.toFixed(2)} AZN`}
//               </span>
//             </Link>
//             {cartProducts.length > 0 && (
//               <div className="absolute -top-1.5 right-3 flex items-center justify-center text-[12px] text-[#ffffff] text-center bg-[#f50809] rounded-full min-w-[16px] h-4 px-1">
//                 {cartProducts.reduce((acc, product) => acc + (product.quantity || 1), 0)}
//               </div>
//             )}
//             <div
//               className="flex flex-col absolute top-12 right-0 bg-[#ffffff] w-[400px] h-auto gap-5 pr-1 z-[100] shadow-xl rounded-md opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 pointer-events-none group-hover:pointer-events-auto transition-all duration-300 ease-in-out"
//             >
//               {cartProducts.length === 0 ? (
//                 <div className="flex items-center justify-center pt-4">
...
//                     {cartProducts.reduce((acc, product) => acc + (product.quantity || 1), 0)} məhsul
//                   </span>
//                 </span>
//               )}
//               <div
...
//               </div>
//               {cartProducts.length > 0 && (
//                 <div className="flex flex-col justify-center gap-2 bg-[#ffffff] px-6 pb-6 rounded">
//                   <p className="text-[14px] text-[#000000] font-normal">
...
//           newVariations.push(variation.substring(0, i) + altChar + variation.substring(i + 1));
//         });
//       });
...
//   const [selectedMainCategoryIndex, setSelectedMainCategoryIndex] = useState(null);
//   const [selectedSubCategoryName, setSelectedSubCategoryName] = useState(null);
//   const [allProductsData, setAllProductsData] = useState([]); // This will now be an array
...
//       console.log("Gələn məhsullar (obyekt):", prodData);

//       const allProductsArray = Object.values(prodData).flat();
//       console.log("Bütün məhsullar (massiv):", allProductsArray);
//       setAllProductsData(allProductsArray);

//       const subcatMap = {};
//       const subsubcatMap = {};

//       Object.keys(prodData).forEach((mainCat) => {
//         const products = prodData[mainCat];
//         const subcats = new Set();
//         const subsubcats = {};
...
//         const authorChar = product.characteristics?.find(char => char.property_id === 12057361);
//         const authorMatch = authorChar && fuzzyIncludes(authorChar.title, searchTerm);
//         const isbnChar = product.characteristics?.find(char => char.property_id === 12019264);
...
//             <div className={`absolute top-21 -left-32 p-1 w-[340px] h-[calc(100vh-100px)] bg-[#ffffff] rounded z-10 overflow-hidden transition-all duration-300 transform ${isHovered ? "opacity-100 translate-y-0 visible" : "opacity-0 translate-y-4 invisible"}`}
//                 style={{ boxShadow: `0 4px 10px rgba(0, 0, 0, 0.1), 0 1px 8px rgba(0, 0, 0, 0.08)` }}>
//               <ul ref={scrollRef} className="py-2 px-4 space-y-1 h-full overflow-y-auto custom-scrollbar">
//                 {Array.isArray(categories) && categories.length > 0 ? (
//                   categories.map((item, index) => {
//                     const hasSubcategories = subcategories[item.category]?.length > 0;
//                     return (
...
//             {activeIndex !== null && categories[activeIndex] && subcategories[categories[activeIndex].category]?.length > 0 && (
//               <div
//                 className="absolute top-21 left-53 w-[960px] h-[calc(100vh-100px)] bg-[#ffffff] rounded z-10 overflow-hidden transition-all duration-300 transform opacity-100 translate-x-0 visible"
//                 style={{
//                   boxShadow: `0 4px 10px rgba(0, 0, 0, 0.1), 0 1px 8px rgba(0, 0, 0, 0.08)`,
//                 }}
//                 onMouseEnter={() => setIsHovered(true)}
...
//             <div
//               ref={inputWrapperRef}
//               className={`relative h-[45px] rounded ${isActive ? "z-60 outline-4 outline-[#ffffff]" : ""}`}
//             >
...
//                   onChange={(e) => setSearchValue(e.target.value)}
//                   onFocus={() => setIsActive(true)}
//                   className="w-full h-full px-4 pr-12 text-[14px] border-2 border-[#f50809] rounded outline-none bg-[#ffffff] relative z-10"
//                 />
//                 {searchValue && (
//                   <button
//                     type="button"
//                     onClick={() => setSearchValue("")}
//                     className="absolute top-0 right-12 h-full w-[50px] flex items-center justify-center text-[18px] text-[#888888] hover:text-[#dc0708] font-bold z-20 cursor-pointer"
//                   >
//                     <CgClose />
//                   </button>
//                 )}
//                 <button
//                   type="submit"
//                   className="absolute top-0 right-0 h-full w-[50px] flex items-center justify-center bg-[#dc0708] rounded-r-md z-20 cursor-pointer"
//                 >
//                   <CgSearch className="text-[25px] text-[#ffffff]" />
//                 </button>
//               </form>
//             </div>

//             {/* Axtarış nəticələri */}
//             {isActive && (searchValue || filteredProducts.length > 0) && (
//               <div
//                 ref={scrollRef}
//                 className="absolute left-0 right-0 max-h-[500px] bg-[#ffffff] outline-4 outline-[#ffffff] rounded shadow-lg overflow-y-auto z-50 custom-scrollbar"
//               >
//                 <div className="pb-12">
//                   {filteredProducts.length > 0 ? (
//                     filteredProducts.map((product) => (
...
//                         className="block"
//                       >
//                         <div className="flex items-center gap-3 w-full h-[90px] p-2 hover:bg-[#f7f8fa] border-b border-[#eeeeee] transition-colors duration-300 cursor-pointer group">
//                           <img
//                             src={
//                               product.images?.[0]?.compact_url ||
...
//                             <span className="text-[16px] text-[#000000] group-hover:text-[#f50809] transition-colors duration-300 font-normal truncate">
//                               {product.title}
//                             </span>
...
//                     ))
//                   ) : (
//                     <div className="p-4 text-center text-[#666666]">Nəticə tapılmadı</div>
//                   )}
//                 </div>

//                 {/* Bütün nəticələr düyməsi */}
//                 <div className="sticky bottom-0 left-0 right-0 bg-[#ffffff] px-2 py-1 rounded-b-md shadow-sm">
//                   <Link
...
//                     <button
//                       type="button"
//                       className="w-full h-[50px] text-[20px] text-[#ffffff] bg-[#0052e6] hover:bg-[#007aff] rounded-md transition-colors duration-300 font-medium"
//                     >
//                       Bütün nəticələr
//                     </button>
//                   </Link>
...
//           <div className="relative w-full z-25">
//             <div ref={inputWrapperRef} className={`relative h-[45px] rounded `}>
//               {/* Form added to handle Enter key submission */}
//               <form onSubmit={handleSearchSubmit} className="w-full h-full">
//                 <input
//                   type="search"
//                   placeholder="Kataloq üzrə axtarış"
...
//                   className="w-full h-full px-4 pr-12 text-[14px] border-2 border-[#f50809] rounded-md outline-none bg-white relative z-10"
//                 />
//                 <button // Changed from div to button for proper form submission
...
// State for mobile favorites and cart
const [isFavoriteOpen, setIsFavoriteOpen] = useState(false);
const handleOpenFavorite = () => setIsFavoriteOpen(true);
const handleCloseFavorite = () => setIsFavoriteOpen(false);
...
const [isScrollable, setIsScrollable] = useState(false);
  ...
  // Cart and liked products for mobile
  const [cartProducts, setCartProducts] = useState([]);
  useEffect(() => {
    const data = JSON.parse(localStorage.getItem("cartProducts")) || [];
    setCartProducts(data);
  ...
  // Function to render cart items in mobile menu
      ...
      <>
        ...
        >
          {cartProducts.map((product) => {
            ...
            return (
              <div key={product.id} className="flex items-center gap-3 w-full">
                <Link to={`/products/${product.permalink}`} onClick={handleCloseCart}>
                  <img
                    src={product.first_image?.medium_url}
                    alt={product.title}
                    className="w-10 h-16 object-cover"
                ...
                <div className="flex flex-col flex-1 self-center">
                  <Link to={`/products/${product.permalink}`} onClick={handleCloseCart} className="text-[12px] text-[#000000] hover:text-[#f50809] transition-colors duration-300 font-normal">
                    {product.title}
                  </Link>
                  <span className="text-[14px] text-[#000000] font-semibold">
                    {quantity} x {unitPrice} AZN
  ...
  // Function to render favorite items in mobile menu
  const renderMobileFavoriteItems = () => {
    ...
    return (
      <>
        ...
        >
          {likedProducts.map((product) => (
            <div key={product.id} className="flex items-center gap-3 w-full">
              <Link to={`/products/${product.permalink}`} onClick={handleCloseFavorite}>
                <img
                  src={product.first_image?.medium_url}
                  alt={product.title}
                  className="w-10 h-16 object-cover"
              ...
              <div className="flex flex-col flex-1 self-center">
                <Link to={`/products/${product.permalink}`} onClick={handleCloseFavorite} className="text-[12px] text-[#000000] hover:text-[#f50809] transition-colors duration-300 font-normal">
                  {product.title}
                </Link>
                <span className="text-[14px] text-[#000000] font-semibold">
                  {product.variants?.[0]?.price} AZN
        ...
        <div className="flex flex-col gap-4 px-6 pb-6">
          <div className="bg-[#f7f8fa] px-6 py-2 rounded-md">
            <Link to="/user" className="text-[12px] text-[#000000] hover:text-[#dc0708] underline cursor-pointer" onClick={handleCloseFavorite}>
              Sevdiyiniz məhsulları saxlamaq üçün şəxsi hesabınıza daxil olun
            </Link>
          </div>
          <Link
    ...
    <>
      ...
      <div className="lg:hidden fixed w-full bg-[#ffffff] h-[50px] top-0 z-[60] shadow flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleOpenFavorite} className="text-[20px] text-[#000000] relative">
            <FaRegHeart />
            {likedProducts.length > 0 && (
              <div className="absolute -top-1.5 right-0 flex items-center justify-center text-[10px] text-[#ffffff] text-center bg-[#f50809] rounded-full min-w-[14px] h-3.5 px-0.5">
                {likedProducts.length}
              </div>
            )}
          </button>
          <button onClick={handleOpenCart} className="text-[20px] text-[#000000] relative">
            <BsCart3 />
            {cartProducts.length > 0 && (
              <div className="absolute -top-1.5 right-0 flex items-center justify-center text-[10px] text-[#ffffff] text-center bg-[#f50809] rounded-full min-w-[14px] h-3.5 px-0.5">
                {cartProducts.reduce((acc, product) => acc + (product.quantity || 1), 0)}
